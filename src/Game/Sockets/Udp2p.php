...
namespace App\Game\Sockets;

use Symfony\Component\EventDispatcher\EventDispatcherInterface;
use Symfony\Component\EventDispatcher\Attribute\AsEventListener;
use Symfony\Component\Stopwatch\Stopwatch;
use App\Game\Sockets\Message\AbstractMessage;
use App\Game\Sockets\Message\AcknowledgeMessage;
use App\Game\Sockets\Message\ConnectionMessage;
use App\Game\Sockets\Message\CriticalMessage;
...
class Udp2p
{
    ...
    private function fireEvent(AbstractMessage $message)
    {
        if ($message instanceof BoardUpdateMessage) {
            $this->eventDispatcher->dispatch(new MultiplayerBoardUpdateEvent($message));
        } elseif ($message instanceof GameOverMessage) {
            $this->eventDispatcher->dispatch(new PeerLoseEvent());
            $this->eventDispatcher->dispatch(new GameOverEvent(GameOverEvent::SOURCE_PEER));
        } elseif ($message instanceof ConnectionMessage) {
            $this->eventDispatcher->dispatch(new PlayerConnectedEvent($this->name, $message->getName()));
        } elseif ($message instanceof AddLinesMessage) {
            $this->eventDispatcher->dispatch(new AddLinesEvent($message->getLines()));
        }
    }
}
